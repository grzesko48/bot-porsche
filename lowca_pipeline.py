...
def render_html(decisions, c, equity_pln=None, free_cash_pln=None, fx=None, held=None,
                sleeve_used_pln=0.0, today="", learn=None, wsreview=None) -> str:
    try:
    ...
    # ZASADY / STOPKA
    # AUDYT WALL STREET — instytucjonalny nadzór portfela rdzenia
    if wsreview and ws:
        try:
            P.append(ws.render_html(wsreview))
        except Exception:
            pass
...
def run(signals_path=None, capital=None, cash=None, fx=None, send=False,
    ...
    # AUDYT WALL STREET — przegląd portfela rdzenia (pozycje z bilansu)
    wsreview = None
    if ws and held:
        try:
            wsreview = ws.review(held, cash if cash is not None else 0.0, capital, fx)
        except Exception as e:
            print(f"Audyt WS pominiety: {e}")
    ...
    if send:
        try:
            ...
            html = render_html(decisions, c, equity_pln=capital, free_cash_pln=cash, fx=fx,
                               held=held, sleeve_used_pln=sleeve_used, today=today, learn=learn, wsreview=wsreview)
