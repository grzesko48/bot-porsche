...
_LADDER_TIGHT  = ((0.05, -0.02), (0.10, 0.00), (0.15, 0.05), (0.25, 0.10),
                  (0.40, 0.20), (0.60, 0.35), (1.00, 0.70))   # obecny (baseline ratchet) — DRABINKA A
...
# DRABINKA B — zagęszczona à la Minervini. Likwiduje "martwą strefę" obecnej drabinki
# (między +10% a +15% zysk niechroniony — break-even przy +10% oddaje cały zysk).
# Inspirowana R-multiple: kolejne progi blokują rosnącą część zysku, bez nagłych skoków.
# Hipoteza: poprawia hit-rate (mniej dobrych pozycji wyrzuconych na korekcie),
# bez utraty edge w długim oknie. Wymaga backtestu na tych samych danych co A.
_LADDER_MINERVINI = ((0.05, -0.02), (0.08, 0.02), (0.12, 0.06), (0.15, 0.10),
                     (0.25, 0.15), (0.40, 0.25), (0.60, 0.40), (1.00, 0.70))

def _sweep_variants() -> "list[tuple[str, BacktestConfig]]":
    """Siatka botów do równoległego backtestu. ŁATWO rozszerzalna: dopisz krotkę.
    AKTUALNY TEST: porównanie drabinki A (obecna, z martwą strefą) vs B (Minervini,
    zagęszczona). WSZYSTKO IDENTYCZNE poza drabinką — izolujemy efekt drabinki.
    Po podjęciu decyzji (B wygrywa / A wygrywa) wracamy do testów rankingu/itd."""
    ...
    return [
        # DRABINKA A — obecna w produkcji (break-even przy +10%, "martwa strefa" +10-15%)
        ("A_obecna_break_even_10",
         BacktestConfig(**base, profit_ladder=_LADDER_TIGHT)),
        # DRABINKA B — zagęszczona Minervini (likwiduje martwą strefę: +8%→+2%, +12%→+6%)
        ("B_minervini_zageszczona",
         BacktestConfig(**base, profit_ladder=_LADDER_MINERVINI)),
        # DRABINKA LOOSE — kontrola: czy bardziej luźna pomaga jeszcze bardziej?
        ("C_loose_oddychanie",
         BacktestConfig(**base, profit_ladder=_LADDER_LOOSE)),
    ]
